...
ATTRIBUTES = [TYPE, RELATED_BY, SOA, NS]
RESTRICT_DOMAIN = ['hostname', TYPE, SOA, NS]


@receiver(post_commit)
def synchronize_powerdns_domains(sender, **kwargs):
    ...
    if kwargs['changed']:
        for changed in kwargs['changed']:
            if _has_powerdns_changes(changed):
...
def _create_domain(hostname: str) -> None:
    ...
    s_object = Query({'hostname': hostname}, RESTRICT_DOMAIN).get()
...
def _has_powerdns_changes(change: dict) -> bool:
    ...
    if not any([attribute in change.keys() for attribute in change]):
        return False
...
def _update_domain(change: dict) -> None:
    ...
    with transaction.atomic():
        ...
        if NS in change:
            for nameserver in change[NS]['remove']:
                Record.objects.filter(
                    domain_id=domain.id, type='NS', content=nameserver
                ).delete()
